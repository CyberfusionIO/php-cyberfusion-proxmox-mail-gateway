...
use Cyberfusion\ProxmoxMGW\Models\MailcountStatistics;
use Cyberfusion\ProxmoxMGW\Requests\MailcountRequest;
use Cyberfusion\ProxmoxMGW\Support\Result;
use Illuminate\Support\Arr;
use Throwable;

/**
 * Endpoint for fetching mail count statistics.
 */
class MailcountEndpoint extends Endpoint {
		/**
		 * Get mail count statistics.
		 *
		 * @param MailcountRequest $request
		 *
		 * @return Result
		 */
		public function get( MailcountRequest $request ): Result {
				try {
						$response = $this->client->makeRequest(
								endpoint: '/statistics/mailcount',
								method  : 'GET',
								params  : [
										'day'       => $request->day,
				...
				foreach ( Arr::get( $data, 'data', [] ) as $stat ) {
						$statistics->push( new MailcountStatistics(
